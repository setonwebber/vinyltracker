...
non-local guesses:
	 -functions/valid/contains.
	 -functions/valid/contains#
	 -functions/valid/contains().
	 -genres/valid/contains.
	 -genres/valid/contains#
	 -genres/valid/contains().
	 -artists/valid/contains.
	 -artists/valid/contains#
	 -artists/valid/contains().
	 -valid/contains.
	 -valid/contains#
	 -valid/contains().
	 -scala/Predef.valid.contains.
	 -scala/Predef.valid.contains#
	 -scala/Predef.valid.contains().
offset: 18082
                    ...
                    while (editing) {
                        ...
                        readLine("> ") match {
                            case "1" =>
                                ...
                                if (vinylName.nonEmpty) vinyl.vinylName = vinylName
                            case "2" =>
                                val vinylType = readLine("Enter new type (EP, LP, Single, Other): ").trim.capitalize
                                val valid = Set("EP", "LP", "Single", "Other")
                                if (valid.conta@@ins(typ)) vinyl.vinylType = vinylType
                                else println("Invalid type.")
                            case "3" =>
                                val releaseDate = readLine("Enter new release date (YYYY-MM-DD): ").trim
                                if (releaseDate.matches("\\d{4}-\\d{2}-\\d{2}")) vinyl.releaseDate = releaseDate
                                else println("Invalid date format.")
                            case "4" =>
                                val condition = readLine("Enter new condition: ").trim
                                if (condition.nonEmpty) vinyl.condition = condition
                            case "5" =>
                                val price = readLine("Enter new price: ").trim
                                try {
                                    vinyl.price = price.toFloat
                                } catch {
