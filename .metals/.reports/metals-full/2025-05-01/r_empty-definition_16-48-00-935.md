...
non-local guesses:
	 -functions/source/getLines.
	 -functions/source/getLines#
	 -functions/source/getLines().
	 -genres/source/getLines.
	 -genres/source/getLines#
	 -genres/source/getLines().
	 -artists/source/getLines.
	 -artists/source/getLines#
	 -artists/source/getLines().
	 -source/getLines.
	 -source/getLines#
	 -source/getLines().
	 -scala/Predef.source.getLines.
	 -scala/Predef.source.getLines#
	 -scala/Predef.source.getLines().
offset: 26542
...
package vinyls{
    ...
    class Vinyls(val artistsObject: Artists, val genresObject: Genres) {
        ...
        def loadVinyls(): Unit = {
            ...
            // if the  vinyls.txtfile exists
            if (os.exists(file)) {
                try {
                    val source = Source.fromFile(file.toIO)
                    val lines = source.get@@Lines().toList
                    source.close()
